...
<?=template_footer()?>
ubuntu@web:/var/www/html/crudpage$ cat delete.php
<?php
include 'functions.php';
...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $stmt->execute([$_GET['id']]);
            $msg = 'Você excluiu o contato com sucesso!';
        } else {
...
<?=template_header('Delete')?>  <!-- Chama o cabeçalho do template -->

<div class="content delete">
    <h2>Excluir Contato #<?=$contact['id']?></h2>
    <?php if ($msg): ?>
        <p><?=$msg?></p>
    <?php else: ?>
...
<?=template_footer()?>  <!-- Chama o rodapé do template -->
